...
// === PATCH: Persistência centralizada de configurações ===
function persistSettings(cfg){
  try { if (cfg) { configuracoes = cfg; } } catch(e){}
  try { localStorage.setItem('settings', JSON.stringify(configuracoes)); } catch(e){}
  try { if (window.__settings && typeof __settings.set==='function') { __settings.set(configuracoes); } } catch(e){}
  try { window.persistSettings = persistSettings; } catch(e){}
  try { console.log('Configurações salvas.'); } catch(e){}
}
...
var viewSlides = "";
var configuracoes = {default_screen: 'maranata', };
...
function atualizaListaArquivos(newData){
  ...
  try { if (window.__saveData) { window.__saveData(dados).catch(()=>{}); } } catch(e) { console.warn("auto-save falhou", e); }
var songList = [];
...
configuracoes = {
  ...
  if (window.__settings && typeof __settings.set==="function") __settings.set(configuracoes);
  
} else {
  try {
    configuracoes = JSON.parse(localStorage.getItem("settings"));
  } catch(e) { configuracoes = {}; }
  // merge defaults if missing fields
  try {
    if (!configuracoes || !configuracoes.themes || !configuracoes.backgrounds) {
      if (typeof defaultConfigurations === 'function') defaultConfigurations();
    }
  } catch(e){}
}
...
// Apply saved default screen selection
try {
  if (configuracoes.default_screen === 'maranata') {
    if (!$('#activeTextStandartScr').is(':checked')) { $('#activeTextStandartScr').trigger('click'); }
  } else if (configuracoes.default_screen === 'santaceia') {
    if (!$('#activeTextStandartScr2').is(':checked')) { $('#activeTextStandartScr2').trigger('click'); }
  }
} catch(e){}
...
$('#confirmConf').click(async function (){
  try {
    if (typeof changeThemeImages === 'function') changeThemeImages();
    if (typeof changeTheme === 'function') changeTheme();
    if (typeof persistSettings === 'function') persistSettings();
  } catch(e) { console.warn('Falha ao aplicar/salvar configs', e); }
  $('#confModal').modal('toggle');
});

function changeTheme(){
  /*GUARD_THEMES*/
  try { if (!configuracoes || !Array.isArray(configuracoes.themes)) return; } catch(e){ return; }
...
function changeThemeImages(){
  /*GUARD_IMAGES*/
  try { if (!configuracoes || !configuracoes.images || !configuracoes.backgrounds) return; } catch(e){ return; }
...
function getBackgroundForSection(id){
  var resp = "";
  try { if (!configuracoes || !Array.isArray(configuracoes.backgrounds)) return resp; } catch(e){ return resp; }
  configuracoes.backgrounds.forEach(function (bg){    
    if(bg.id == id){         
      if(bg.type == 1){        
...
function getBackground(id){
  var oldbg = {};
  try { if (!configuracoes || !Array.isArray(configuracoes.backgrounds)) return oldbg; } catch(e){ return oldbg; }
  configuracoes.backgrounds.forEach(function (bg){    
    if(bg.id == id){ 
      oldbg = bg;
    }     
