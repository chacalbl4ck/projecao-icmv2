...
/* === PATCH: salvar data.json automático (backend → FS Access → download) === */
window.__saveData = async function __saveData(dados){
  try {
    if (!window.__fsSave || !window.__fsSave.saveDataJson) {
      console.warn('fs-save não carregado; fallback para download.');
    ...
    await window.__fsSave.saveDataJson(dados);
  } catch (e) {
    console.error('Falha ao salvar dados', e);
  }
};

/* === PATCH: persistência de configurações === */
window.__applyAndSaveSettings = function(cfg){
  try {
    if (window.__settings && typeof window.__settings.set === 'function') {
      window.__settings.set(cfg);
    }
  } catch(e){ console.warn('Não foi possível salvar settings', e); }
  try {
    if (typeof aplicarConfiguracoesNaView === 'function') {
      aplicarConfiguracoesNaView(cfg);
    } else if (window.postMessage) {
      // fallback: envia para view
      const target = window.open('', 'projection-view') || window.opener || window.parent;
      if (target && target.postMessage) {
        target.postMessage({ type: 'APPLY_SETTINGS', payload: cfg }, '*');
      }
    }
  } catch(e){ console.warn('Não foi possível aplicar settings na view', e); }
};

/* === PATCH: carregar configurações no startup === */
(function(){
  try {
    if (window.__settings && typeof window.__settings.get === 'function') {
      const cfg = window.__settings.get();
      if (cfg && Object.keys(cfg).length) {
        if (typeof preencherFormularioCom === 'function') preencherFormularioCom(cfg);
        if (typeof aplicarConfiguracoesNaView === 'function') aplicarConfiguracoesNaView(cfg);
      }
    }
  } catch(e){ console.warn('Startup settings falhou', e); }
})();
